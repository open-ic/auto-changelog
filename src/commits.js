...
const DEFAULT_FIX_PATTERN =
  /(?:close[sd]?|fixe?[sd]?|resolve[sd]?)\s(?:#(\d+)|(https?:\/\/.+?\/(?:issues|pull|pull-requests|merge_requests)\/(\d+)))/gi;
...
const parseCommits = async (string, options = {}) => {
  const parsed = string
    .split(COMMIT_SEPARATOR)
    .slice(1)
    .map((commit) => parseCommit(commit, options))
    .filter((commit) => {
      return filterCommit(commit, options);
    });

  if (options.filePattern) {
    const promises = parsed.map((commit) => {
      return fetchFiles(options.filePattern, commit);
    });
    const results = await Promise.all(promises);
    return parsed.filter((_, idx) => results[idx]);
  }
...
const fetchFiles = async (pattern, { hash }) => {
  const files = await cmd(`git show --name-only ${hash}`);
  if (files) {
    try {
      const [, _hash, _author, _date, , _msg, filestr] =
        files.match(MATCH_FILES);
      const fileList = filestr.split(/\n/);
      const match = fileList
        .filter((file) => file !== "")
        .reduce((match, file) => {
          return match || new RegExp(pattern).test(file);
        }, false);
      return match;
    } catch (err) {
      return false;
